...
class DomainSampler(ABC):
    ...
    def sample_domain_boundary(self, n_points: int) -> torch.Tensor:
        """
        Sample points uniformly on the outer faces of the (hyper-)rectangular domain.

        Points are split evenly across the 2 * spatial_dim faces; any remainder is
        truncated so exactly ``n_points`` (or 0) are returned.

        Returns:
            Tensor of shape [n_points, spatial_dim]
        """
        if n_points <= 0:
            return torch.empty(0, self.spatial_dim, device=self.device, dtype=self.dtype)

        points_per_face = -(-n_points // (2 * self.spatial_dim))  # ceil division
        faces = []
        for dim in range(self.spatial_dim):
            for boundary_val in self.domain_bounds[dim]:
                face_points = self._uniform_sample_in_bounds(points_per_face)
                face_points[:, dim] = boundary_val
                faces.append(face_points)

        return torch.cat(faces, dim=0)[:n_points]
...
class UniformSampler(DomainSampler):
    ...
    def sample_points(self, n_points: int, **kwargs) -> Dict[str, torch.Tensor]:
        ...
        if len(points) < n_points:
            warnings.warn(f"Could only generate {len(points)} points instead of {n_points}", stacklevel=2)
    ...
    def _sample_interface_points(self, n_points: int) -> torch.Tensor:
        ...
        for interface in self.interfaces:
            ...
            if interface_type == 'plane':
                ...
                warnings.warn(f"Unknown interface type: {interface_type}", stacklevel=2)
                continue
...
class AdaptiveSampler(DomainSampler):
    ...
    def refine_around_high_residuals(self,
                                     n_new_points: int,
                                     residual_threshold: float = 0.9,
                                     radius_factor: float = 0.1) -> torch.Tensor:
        """
        Generate new points clustered around the highest-residual locations.

        Uses the most recent entry recorded via ``update_residuals``. Points whose
        residual magnitude exceeds the ``residual_threshold`` quantile act as
        centres; new points are drawn from Gaussians of width
        ``radius_factor * min(domain extent)`` about them and clamped to the
        domain. Falls back to the base sampler when no residuals are available.

        Returns:
            Tensor of shape [n_new_points, spatial_dim]
        """
        if n_new_points <= 0:
            return torch.empty(0, self.spatial_dim, device=self.device, dtype=self.dtype)

        if not self.residual_history:
            return self.base_sampler.sample_points(n_new_points)['points']

        recent = self.residual_history[-1]
        points = recent['points']
        residuals = recent['residuals']
        residual_mag = torch.norm(residuals, dim=1) if residuals.dim() > 1 else torch.abs(residuals)
        threshold_val = torch.quantile(residual_mag, residual_threshold)
        centres = points[residual_mag > threshold_val]

        if len(centres) == 0:
            return self.base_sampler.sample_points(n_new_points)['points']

        lows = torch.tensor([lo for lo, _ in self.domain_bounds], dtype=self.dtype)
        highs = torch.tensor([hi for _, hi in self.domain_bounds], dtype=self.dtype)
        radius = radius_factor * torch.min(highs - lows)

        per_centre = max(1, n_new_points // len(centres))
        centres = centres.to(self.dtype)
        offsets = torch.randn(len(centres), per_centre, self.spatial_dim, dtype=self.dtype) * radius
        new_points = (centres.unsqueeze(1) + offsets).reshape(-1, self.spatial_dim)
        new_points = torch.max(torch.min(new_points, highs), lows)

        remaining = n_new_points - len(new_points)
        if remaining > 0:
            extra = self.base_sampler.sample_points(remaining)['points'].cpu().to(self.dtype)
            new_points = torch.cat([new_points, extra], dim=0)

        return new_points[:n_new_points].to(device=self.device)
