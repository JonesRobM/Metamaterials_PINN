...
from src.models.field_format import to_complex
...
def _fields_from_network(network, coords):
    """Return complex ``(E, H)`` [N,3] tensors from any supported network output."""
    if hasattr(network, 'get_fields'):
        return network.get_fields(coords)
    fields = network(coords)
    if fields.dim() == 3 and fields.shape[-1] == 2:  # [N, 6, 2] real/imag layout
        return to_complex(fields)
    if fields.is_complex():
        return fields[:, :3], fields[:, 3:6]
    # Flat real layout [N, 12]: (Re E, Im E, Re H, Im H)
    return (torch.complex(fields[:, :3], fields[:, 3:6]),
            torch.complex(fields[:, 6:9], fields[:, 9:12]))
...
class MaxwellResidualMetrics(BaseMetric):
    ...
    def compute(self,
        ...
        E_field, H_field = _fields_from_network(network, coords)
...
class BoundaryConditionMetrics(BaseMetric):
    ...
    def compute(self,
        ...
        E_field, H_field = _fields_from_network(network, interface_coords)

        results = {}

        if self.boundary_solver:
            # Use physics module
            torch.eye(3, dtype=torch.complex64, device=self.device)\
...
class SPPPhysicsMetrics(BaseMetric):
    ...
    def compute(self,
        ...
        # Get fields
        E_field, _H_field = _fields_from_network(network, coords)
        ...
        if self.expected_k_spp and metamaterial_props:
            try:
                ...
                warnings.warn(f"Could not compute dispersion accuracy: {e}", stacklevel=2)
    ...
    def _estimate_wavevector(self, coords: torch.Tensor, E_field: torch.Tensor) -> complex:
        ...
        torch.cumsum(torch.cat([phase[:1], phase_diff]), dim=0)
...
class EnergyConservationMetrics(BaseMetric):
    ...
    def compute(self,
        ...
        try:
            ...
            warnings.warn(f"Could not compute energy metrics: {e}", stacklevel=2)
...
class MetricsCollector:
    ...
    def evaluate_all(self,
        ...
        try:
            ...
            warnings.warn(f"Maxwell metrics failed: {e!r}", RuntimeWarning, stacklevel=2)
            results['maxwell'] = {}
        ...
        try:
            ...
            warnings.warn(f"SPP metrics failed: {e!r}", RuntimeWarning, stacklevel=2)
            results['spp'] = {}
        ...
        try:
            ...
            warnings.warn(f"Energy metrics failed: {e!r}", RuntimeWarning, stacklevel=2)
            results['energy'] = {}
    ...
    def get_summary_report(self, recent_epochs: int = 10) -> str:
        ...
        if maxwell_losses:
            report.append("Maxwell Residuals:")
