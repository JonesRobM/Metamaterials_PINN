...
class EMFieldPlotter(BasePlotter):
    ...
    def _fit_exponential_decay(self, ax, coords, field_values, interface_pos, region_name):
        ...
        try:
            ...
            warnings.warn(f"Could not fit decay in {region_name}: {e}", stacklevel=2)
...
class DispersionPlotter(BasePlotter):
    ...
    def plot_isofrequency_contours(self,
        ...
        # Contour plot
        ax.contour(kx_grid * 1e-6, ky_grid * 1e-6, frequency_map * 1e-15,
...
class InteractivePlotter:
    ...
    def __init__(self):
        if not PLOTLY_AVAILABLE:
            warnings.warn("Plotly not available. Interactive plots disabled.", stacklevel=2)
