"""Validate the physics module (Maxwell residuals, metamaterial dispersion, boundary conditions).
...
from src.physics import BoundaryConditions, MaxwellEquations, MetamaterialProperties  # noqa: E402
...
def validate_maxwell_equations():
    ...
    # Wavevector must satisfy the vacuum dispersion relation |k| = omega / c
    k0 = omega / C0
    k = torch.tensor([k0, 0, 0], dtype=torch.float32)  # Wavevector in x-direction
    ...
    k_cross_E = torch.linalg.cross(k_expanded, E_field.real, dim=1) + 1j * torch.linalg.cross(
        k_expanded, E_field.imag, dim=1
    )
    ...
    print(f"Maximum Maxwell residual: {max_residual:.2e} (relative to k0: {max_residual / k0:.2e})")

    # Residual is scaled relative to the field magnitude (|E| = 1, |k| ~ 1e7)
    if max_residual / k0 < 1e-3:
...
def validate_metamaterial_properties():
    ...
    # Type-II hyperbolic medium (ε_t < 0 in-plane, ε_n > 0 along the z optical
    # axis): this is the orientation that binds a TM surface wave. The reverse
    # (ε_t > 0) is radiative and correctly reported as unsupported.
    eps_par = 4.0 + 0.05j
    eps_perp = -2.0 + 0.1j
...
def validate_boundary_conditions():
    ...
    2.0 * torch.eye(3, dtype=torch.complex64).unsqueeze(0).expand(batch_size, -1, -1)
